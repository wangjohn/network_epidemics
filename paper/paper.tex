...
\begin{equation}
	1 - s = e^{-(n(1-q)-1)ps}
\end{equation}
Computing an analytical solution to this equation is extremely difficult, so we will determine $s(q, n)$ numerically in the next section. 
...
In this section, we provide results from computational simulations of our model. The graphs that we simulate on are created using the Erd\H{o}s-R\'{e}nyi mechanism with the number of nodes fixed at $n = 100$. Fixing the number of nodes reduces the number of parameters that can vary.
...
Because we cannot compute the equilibrium vector of security $\bar{q}$ where $q_i \in [0,1)$ is player $i$'s choice of security, we can sweep along a grid of representative values for $\bar{q}$ and examine the game at each one of these values.

Fortunately, this game is symmetric for each player and Acemoglu et al (2013) have shown that under such situations, the Nash Equilibrium for each player is to play the same security $q$. Therefore it suffices to check setting $q_i = q_j$ for each player $i \neq j$ in our simulations. We shall call $q = q_i$ the security that each player chooses.

To examine the outcome of a contagion spreading on an Erd\H{o}s-R\'{e}nyi graph with edge probability $p$, $n = 100$, and security rate $q$ for each participant, we must be able to compute $\tilde{P}_i(A, q_{-i}, \Phi)$, the probability that the infection reaches $i$ (i.e. the probability that any one of $i$'s neighbors gets infected). However, since computing $\tilde{P}_i$ is NP-hard, we instead use Monte Carlo simulations to approximate $\tilde{P}_i$. We do so in the following manner:

\begin{itemize}
  \item Generate $\kappa$ Erd\H{o}s-R\'{e}nyi graphs with parameters $G(n, p)$.
  \item For each graph instance and security value $q$, we run the following for $\gamma$ trials: choose a random node to infect and run the infection to completion.
...
We use the above model to simulate results, with $n = 100$, $\gamma = 250$, and $\kappa = 100$. In order words, we generate $\kappa = 100$ random graphs for each set of Erd\H{o}s-R\'{e}nyi parameters, then on each graph, we run $\gamma = 250$ distinct infections which each start at a random node. The results are then summarized for different security rates of $q$.

We begin by presenting the average infection probability $\hat{P}$ when the edge creation probability in Erd\H{o}s-R\'{e}nyi is very small (we choose $p = 0.005$). Here the Erd\H{o}s-R\'{e}nyi parameter is $np = 0.5$, which is below the threshold value for the existence of a giant component. Under this regime, the average infection probability $\hat{P}$ is very low for all security levels (even a security rate of $q = 0.05$), as seen in the simulation results from figure \ref{avg_prob_np05.fig}.

The intuition behind this is clear. Since $np = 0.5 < 1$, the average size of a component is small, so the amount that the infection can spread is decreased. Thus, even for low security rates, the infection cannot spread very far.
...
When the Erd\H{o}s-R\'{e}nyi parameter predicts the presence of a giant component, the average infection rate has a strong dependence on the security rate. We use $np = 5 > 1$ which should produce a giant component with high probability, and simulate the resulting infection probabilities for various values of $q$. Figure \ref{avg_prob_np5.fig} shows the results from our simulations for $np = 5$. For low $q$, the average infection rate is incredibly high. For example, when $q = 0.05$, the average infection rate is $\hat{P} = 0.93$ with a standard deviation of $0.0103$.

As the security rate increases, the average infection probability decreases almost linearly until about $q = 0.6$. Once $q = 0.6$, the positive externalities start to kick in and the average infection probability drops off more dramatically than the linear trend would predict. This can be attributed to the "network effect", or the extra benefit that everyone obtains from one person increasing their security. Having higher $q$ will positively impact your neighbors because of the lower probability of you getting infected and thus transmitting to your neighbors. 
...
The network effect starts to increase to something significant by $q = 0.5$, then falls off again after $q = 0.8$. The fall off in the network effect comes from the saturation of security levels. After a certain point, the marginal benefit of extra security starts to decrease because of the difficulty of actually spreading the infection.
...
Agents are fully informed of the attack and cure probabilities, as well as the security levels chosen by the nodes in their neighborhood in the previous timestep. The attack probability, denoted by  $\alpha$, is the probability of exogenous attack on a random node at each timestep. Similarly, the cure probability is analogous to the recovery rate $\gamma$, i.e. the probability that a currently infected node will no longer be infected at the next timestep.
...
For dynamic simulations, we use a cost function of $c(q) = q^2$. Given that the cure probability is $\gamma$ and the attack probability is $\alpha$, the probability $\tilde{\textbf{P}}(A,q_{-i})$, given the network $A$ and the security profile $q_{-i}$, that the infection reaches agent $i$ is:
\begin{equation}
...
We find that in the dynamic model, the initial choice of security level for all of the nodes makes little difference in the resulting infection probability of the graph (Fig. \ref{fig:attack_cure}. Intuitively, this makes sense, since over many timesteps where players update their choice of security, it will converge to some relatively stable security profile, with changes from attacks and cures cancelling. The results suggest that in the long-term, if attack and cure probabilities remain stable, a population of the graph will remain infected, the size of which depends on the choices of $\gamma$ and $\alpha$.
...
\begin{figure}[h!]
  ...
  \caption{$\alpha$ is fixed for the HIV model at $0.05$, with preferential attachment, and $\gamma$ is varied. a) shows the average infection probability for different initial security levels and b) averages the infection probability over the different security levels and compares to the choice of $\gamma$.}
  \label{fig:hiv}
\end{figure}
...
\begin{theorem}
  Computing $\tilde{P}_i(A, q_{-i}, \Phi)$ for all $i \in \{1, \ldots, n\}$ on an Erd\H{o}s-R\'{e}nyi graph with edge probability $p$ is $\# P$ hard in the static model.
\end{theorem}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Set the security rates $q_e \in [0,1)$ for each edge $e \in E$ for the Epidemic Probability Problem as $1 - Pr(e)$, where $Pr(e)$ is the loss probability for edge $e$ in the Two Terminal Problem.
...
The complexity result can be extended to the dynamic case when the security for each node potentially changes. This means that for both the static and the dynamic model, the computation of $\tilde{P}_i$ is computationally intractable.

\begin{corollary}
  Computing $\tilde{P}_i(A, q_{-i}, \Phi)$ for all $i \in \{1, \ldots, n\}$ on an Erd\H{o}s-R\'{e}nyi graph with edge probability $p$ is $\# P$ hard is the dynamic model with changes in security in each time step.
\end{corollary}
\begin{proof}
...
\begin{theorem}
  Computing the expected number of infected nodes on the an epidemic network with security $q_i$ for $i \in \{1, \ldots, n\}$ is $\# P$ hard.
\end{theorem}
\begin{proof}
  ...
  Suppose we have an instance of the Epidemic Probability Problem. Now select a node $u \in V$ and create a new node $v \not \in V$ which will be attached to the graph. Now, create an edge $e(u,v)$ between $u$ and $v$ and set $v$'s security to $q = 0$. Now, if $u$ is ever infected, then $v$ will also be infected.
...
\begin{definition}
    We will be given a graph $G = (V,E)$ and some function of security $Q: E \to [0,1)$ which defines the security $q_v$ inherent for the infection to move down edge $(u,v)$. The \emph{MSMSIP problem} is to find the probability of infection $P_i(u)$ for each node $i \in V \setminus u$ for all possible starting nodes $u \in V$.
  \end{definition}
...
Karger solves the all terminal problem using a combination of a minimum cut finding algorithm and the Monte Carlo scheme. If $\tilde{P}_i$ is large, then it is sufficient to use the Monte Carlo algorithm because of the time bound of $O(\log n / (\epsilon^2 \tilde{P}))$. However, when $\tilde{P}$ is small and arbitrarily close to zero, the Monte Carlo simulation requires a prohibitive number of trials. Therefore, Karger develops another polynomial time algorithm which can be used when $\tilde{P} < 1/n^4$ (for explanation, we assume that all nodes have security rate $q$, but this can be extended to each node having different security rates without much difficulty). When this is the case, we can find cuts that are small enough, and these cuts will provide an easy way to estimate the probability of failure.
...
We found a number of interesting problems related to our model were computationally intractable. For example, computing the probability of infection given a vector of security for each node is NP-hard. However, we developed a polynomial time approximation scheme which can compute infection probabilities within an arbitrarily small range of their true values.

Our results show that purely selfish play on our model is worst (in comparison to the social optimal) when the network has an intermediate level of connectivity. Moreover, security investments make no difference when the graph is disconnected, but has a large impact on slowing the spread of a disease when the graph is connected.

This paper introduces a number of techniques for analyzing infection spread in networks. Our characterization of infection spread on Erd\H{o}s-R\'{e}nyi graphs hopefully provides clearer insight into how infections spread in physical systems.
...
\begin{thebibliography}{9}
  ...
  \bibitem{acemoglu13}
    D. Acemoglu, A. Malekian, and A. Ozdaglar.
    Network Security and Contagion.
    \emph{Unpublished}, 2013.
  ...
  \bibitem{newman10}
    ...
    \emph{Oxford University Press}, 2010.
